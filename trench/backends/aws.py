...
from trench.settings import AWS_ACCESS_KEY, AWS_SECRET_KEY, AWS_REGION, AWS_FROM_NUMBER
...
class AWSMessageDispatcher(AbstractMessageDispatcher):
    ...
    def dispatch_message(self) -> DispatchResponse:
        try:
            ...
            from_number = self._config.get(AWS_FROM_NUMBER)
            client.publish(
        ...
        except botocore.exceptions.ClientError as cause:
            logging.error(cause, exc_info=True)
            return FailedDispatchResponse(details=str(cause))
        except botocore.exceptions.EndpointConnectionError as cause:
            logging.error(cause, exc_info=True)
            return FailedDispatchResponse(details=str(cause))
